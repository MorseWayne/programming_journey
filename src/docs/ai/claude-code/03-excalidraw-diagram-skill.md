...
[`coleam00/excalidraw-diagram-skill`](https://github.com/coleam00/excalidraw-diagram-skill) 是一个面向 Claude Code 的 diagram skill。它不只是生成 `.excalidraw` JSON，而是要求 agent 先设计图的表达逻辑，再渲染成 PNG 检查效果，最后根据截图修正布局。
...
Claude Code 的 skill 可以放在用户级目录，也可以放在项目级目录。

| 位置 | 作用域 | 适合场景 |
|---|---|---|
| `~/.claude/skills/excalidraw-diagram/` | 当前用户全局可用 | 个人长期使用 |
| `.claude/skills/excalidraw-diagram/` | 当前项目可用 | 团队共享同一套画图规范 |
...
这样其他人拉取仓库后，也能看到同一份 skill 说明、颜色规范和渲染脚本。
...
配置好 skill 后，可以直接在 Claude Code 中描述你要的图：
