...
| [04 mattpocock/skills 安装与迁移指南](./04-mattpocock-skills-install-migration.md) | 如何用 Skills CLI 安装社区 skills，并把 `~/.agents/skills` 迁移到 Claude Code？ | 想统一管理 agent skills 的用户 |
...
>
> 对社区 skills，`mattpocock/skills` 更适合作为一组工程工作流入口：先用 Skills CLI 安装，再按仓库运行 setup；如果本机已经有 `~/.agents/skills`，可以用软链接迁移到 Claude Code，避免复制后产生两份内容。
