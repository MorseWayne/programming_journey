...
它们都能安全遍历、读取长度和 `append`。一般没有元素时直接使用 `nil` 切片即可；与 JSON 等接口交互时才可能需要区分：`nil` 常编码为 `null`，空切片常编码为 `[]`。
...
```go
// append(nums, 3) // 返回的新切片没有被保存，通常没有意义
```
...
其中 `max` 限制新切片的容量终点。上例中容量被限制为 2，因此再追加时必须创建新数组。
...
遍历同一个切片时，避免随意 `append`、删除或重排它，行为会难以直观推断。
