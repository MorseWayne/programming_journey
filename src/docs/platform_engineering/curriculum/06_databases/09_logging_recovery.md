---
title: 06.09 日志与崩溃恢复：消息提交后还能找回吗
icon: /assets/icons/article.svg
order: 10
date: 2026-09-24
---

[返回第六卷](./README.md) · [缓冲页前置：06.04](./04_pages_buffer_pool.md) · [事务前置：06.07](./07_transactions_anomalies.md) · [锁与版本前置：06.08](./08_locks_mvcc.md) · [文件持久前置：03.08](../03_systems/08_persistence_mechanisms.md)
...
操作层面要区分两个目标：**RPO** 是可接受的最大数据丢失范围，**RTO** 是从故障到恢复服务的目标时长。更频繁检查点可能改变恢复工作量，但不替代明确的备份、WAL 归档、恢复演练、跨节点故障方案；也不能凭“有副本”推断有人误删整段历史后能找回。备份恢复/时间点恢复要有完整链条并实际演练，[06.11](./11_replication_migration_reconciliation.md)再接复制迁移。[PostgreSQL WAL 与时间点恢复](https://www.postgresql.org/docs/current/wal-intro.html)
...
按[学习路线](../learning_path.md)，下一章[06.11 复制、迁移与对账](./11_replication_migration_reconciliation.md)将把单机提交扩展到副本、迁移和对账；06.12 再做完整消息存储案例。
