---
title: 06.01 关系模型与业务身份：从会话和消息画出第一张数据表
icon: /assets/icons/article.svg
order: 2
date: 2026-09-24
---

[返回第六卷](./README.md) · [数据前置：01.05](../01_go/05_maps_structs_pointers.md) · [业务前置：09.01](../09_backend_security/01_requirements_boundaries.md) · [测量边界：11.01](../11_reliability/01_business_measurement.md)
...
下一章[06.02 SQL 从读写到复杂查询](./02_sql_queries.md)才从 `SELECT`、`INSERT`、`UPDATE`、`DELETE` 写到连接、聚合和分页。离开本章前，请能不用 SQL 说清 `m-a` 是谁、属于谁、怎样查到、哪种约束能保护它，以及**哪些权限与确认仍不受这张表保护**。
