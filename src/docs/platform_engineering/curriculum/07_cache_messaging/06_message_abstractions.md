---
title: 07.06 消息抽象：在线提示、后台任务与可回放日志
icon: /assets/icons/article.svg
order: 7
date: 2026-09-24
---

[返回第七卷](./README.md) · [状态角色前置：07.01](./01_access_state_roles.md) · [Redis Stream 前置：07.02](./02_redis_data_model.md) · [数据库事实前置：06.12](../06_databases/12_database_business_case.md) · [部分失败前置：08.01](../08_distributed/01_system_partial_failure.md)
...
承接 06.12 的**未来 S3 教学情景**：`m-9` 已作为 `c-a.seq=9` 在权威消息数据库中已知提交。之后有三件不同的事：若 B 在线，可**提示**它补拉；后台搜索索引要更新；B 若离线很久，之后仍要能查历史。数据库里的 `m-9` 是**业务消息事实**，事件 **E9** 是告诉后续组件“这份事实值得处理”的**处理通知**。E9 即使在某中间件里可保存，也不会因叫 message 就自动变成消息正文的唯一权威来源。[07.01 状态角色](./01_access_state_roles.md)
...
“发布命令成功”最多说明中间件接受了该发布动作，不能从中推导 B 的网络连接收到、应用处理、界面展示或用户阅读。即使 G 得到某个在线订阅数量，也不是每台目标设备的应用确认。若产品需求要求离线通知保留、可靠推送或跨节点重试，就要另设可回放/确认路径与设备级证据；不能把 Pub/Sub 的实时广播称为离线信箱。[09.12 完整服务项目](../09_backend_security/12_im_service_capstone.md)
...
日志适合在有限保留期内重建搜索、会话摘要等派生状态，但重放可能重复触发副作用。消费者不能把“重新读 E9”自动解释为“再给 B 发一条新聊天消息”；应按 m-9/version 识别同一权威事实。生产者确认也有故障窗口：发布请求超时可能已被 broker 接纳，是否重复发布、怎样去重，要在 07.07 继续。[08.01 部分失败](../08_distributed/01_system_partial_failure.md)
...
按[学习路线](../learning_path.md)，下一章[07.07 确认、重投与去重](./07_ack_retry_dedup.md)将分别推演生产确认、消费确认、重投和去重窗口；07.08 再深入同会话顺序与并发消费。
