...
能说明低 CPU 为何仍可能有长等待，手算连接池两种均值与设备队列净积压，区分缓存/提交、`Write`/TCP ACK/应用 ACK，画出 B 迟到的分段反证矩阵，并为慢端隔离写出权威历史补拉前提，才算完成第一轮。所有结果均是纸上推演，真实验证须在学习者的隔离实现记录具体版本、负载和授权证据。下一章 11.07 将用可控负载与容量实验检验这些候选。
