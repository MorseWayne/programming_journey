...
能在本章图上区分最少跳数与最低代价，逐行复算 Dijkstra 的松弛、定型和 parent，给出不可达 F 与负权反例，并说明带 broker 额度时必须扩展状态，才算完成本章。下一章 02.10 从“能拆成哪些子问题”进入动态规划和贪心，继续用有限 IM 预算解释算法条件。[第二卷路线](./README.md)
