...
能画 A→P→N1/N2 的两段连接与 TLS/头部信任边界，解释健康探测和粘性为什么不改变 S2 内存语义，并在 1000 ms 纸上预算里识别代理 504 后的未知结果与同 ID 409 重试风险，才算完成本章。下一章 04.10 将比较 HTTP 版本、流控和内部 RPC 的错误分层。[第四卷路线](./README.md)
