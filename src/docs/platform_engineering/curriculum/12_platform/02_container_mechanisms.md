...
能说明同镜像 A/B 的内存与可写层为何独立，分清停止再启和删除重建对内存/文件/卷的不同影响，指出 namespace/cgroup、PID 1 信号及 WebSocket 排空边界，再用当前 `/v1` 正反例核对业务，才算完成第一轮。本章没有实际启动任何容器。下一章[12.03 本地多服务环境](./03_local_multi_service.md)将在纸上环境中解释服务名、端口、配置、健康与依赖就绪。
