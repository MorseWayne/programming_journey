...
能手算三副本的 surge/unavailable 预算，区分金丝雀请求、连接和用户曝光，说明 P4 Pending 时为何不能盲删 P1，并为 D1/D2、K1/K2、数据/事件/客户端分别写停止与回退门，才算完成第一轮。本章未发布任何制品。下一章 12.10 将分析扩缩容指标滞后、节点/可用区故障域和重连尖峰的 N−1 余量。
