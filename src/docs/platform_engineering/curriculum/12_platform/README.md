...
| 12.11 声明式交付与可重复环境 | 配置分层、模板、GitOps 思想、漂移、权限与审计。 | 从一次IM配置变更追踪到制品、权限和运行状态，解释漂移与审计。 |
