...
| [05.12 并发服务复盘](./12_concurrency_service_review.md) | 热点、级联等待、取消失效、内存膨胀和退出失败。 | 复盘一次慢设备、大群积压或取消失效，给出正常、过载、退出与恢复路径。 |
