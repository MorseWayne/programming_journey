...
能不看答案手算三键落桶，解释为什么虚拟桶让**桶搬迁**可控却拆不开 `c-g` 单热键；能用 M0–M6 写出单写者、快照增量、epoch 围栏、E9/E10 顺序、旧客户端与回退条件，并将数据库搬迁与 P0 消费者再平衡分开，才算完成第一轮。第二轮在自己的隔离环境按真实配置演练，记录差集与失败证据。下一章 08.06「多数与共识」将从任期、投票和复制日志继续追问谁有写权，以及何时能承诺一条记录已提交。
