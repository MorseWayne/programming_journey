...
能用 Python 对象、函数、异常和 `pathlib/json` 解释纸上 JSONL 管线，按 UTF-8 字节与字段规则拒绝不合格行，区分样本重复与线上 409，并交付无真实数据、权限状态清楚的复现清单，才算完成第一轮。下一章 14.02 将从向量、相似度、概率和训练/验证的手算直觉进入机器学习前置。
