---
title: 01.03 控制流与函数：把 IM 消息规则写清楚
icon: /assets/icons/article.svg
order: 4
date: 2026-09-23
---

[返回第一卷](./README.md) · [上一章：类型与数据表示](./02_types_data.md)

本章示例给出按规则推导的预期结果；学习时请逐步核对，并记录自己的实际结果。

## 本章怎样接上前面的知识

先修为 [01.01 程序与工具链](./01_program_toolchain.md)和 [01.02 类型与数据表示](./02_types_data.md)。你已经见过变量、类型、比较、逻辑条件、字符串、`len`、最小 `if` 和 `main` 中的 `return`。现在把这些能力放进一个 IM 场景，再逐步加入循环与函数。
...
上游函数还使用指针、结构体、协议序列化和 RPC，这些不属于本章先修。现在不要求逐字读懂整份函数；后续在 01.05、01.07、04 和 09 卷讲清后再沿同一入口深入。主项目的整体位置见 [IM 阅读地图](../im_reference.md)。
...
下一章 [01.04 数组、切片与文本](./04_collections_text.md) 将把固定单条输入扩展为多条消息，讲清列表、下标、底层共享、复制与文本边界。此时再回到循环，就能区分“重复处理一个固定值”和“遍历一组不同消息”。
