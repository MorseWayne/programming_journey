---
title: C11 生命周期与发布：从进程启动到业务可用
icon: /assets/icons/article.svg
order: 11
date: 2026-09-22
---

[C10–C12 单元导读](./stages/05_platform.md) · 前置：[取消](./03_concurrency.md)、[恢复](./07_cache_recovery.md)、[归属](./09_distributed_state.md)、[指标](./10_observability.md)
...
| 已学内容 | 本课用它做什么 |
|---|---|
| [B04 容器与配置](./backend_basics/04_local_tools.md) | 理解镜像、端口与 volume |
| [B07 取消](./backend_basics/07_channels_context.md) | 知道退出需要协作 |
| [C07 恢复](./07_cache_recovery.md) | 区分退出与持久性 |

## 需求场景：更新镜像之后，匹配短暂异常

发布系统显示滚动更新完成，但用户请求仍可能落到旧节点，新的协作空间 owner 也可能尚未恢复状态。进程、路由、状态与用户结果各有自己的变化时间。

本课先理解部署对象和探针，再推导可解释的启动与退出顺序。第一轮在本地完成 HTTP 实验，具备环境后再进入集群实践。

## 基础理论：镜像、容器、Pod 与服务

B04 已讲过 Docker 与 Compose。Kubernetes 是用于管理容器运行、调度和服务访问的系统；kubectl 是与它交互的命令行工具；kind 可以在个人本地使用容器准备学习集群。集群是一组共同承担运行职责的节点。

这里的 namespace 是集群资源的命名范围，context 是客户端选择哪组集群连接与身份配置；它们与业务请求里的团队 ID 不同。

镜像封装程序与运行所需文件；容器是基于镜像启动的受隔离进程环境。容器重启不等于内存继续存在，本地文件是否保留则取决于实际存储方式。

Pod 是 Kubernetes 调度和管理的一组容器运行单元。Deployment 维护期望的副本与更新过程；Service 提供访问后端实例的抽象。它们解决部署和路由的一部分问题，不会替应用共享余额或恢复内存协作空间。
...
这是需要协调的责任关系，并非对所有架构都适用的固定命令顺序。终止宽限期提供时间预算，超时后仍可能被强制结束，因此C07 课的异常恢复不可省略。
