...
- [09.01 用例合同](../../../src/docs/platform_engineering/curriculum/09_backend_security/01_requirements_boundaries.md)与[09.02 HTTP 合同](../../../src/docs/platform_engineering/curriculum/09_backend_security/02_http_api_contract.md)是本章所有目录和错误映射的课程依据。

下一章[09.04 请求处理链](../../../src/docs/platform_engineering/curriculum/09_backend_security/04_request_pipeline.md)将把请求进入、认证/授权、应用调用、取消和响应提交放进同一条处理链；进入前应能从 handler 追到内存适配器，再带着错误身份回到唯一的 HTTP 映射点。
