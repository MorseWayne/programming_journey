...
2. HTTP 请求体、解码后的正文和附件各自设上限。`Content-Length` 只能辅助早判，实际读取必须有界；Go `MaxBytesReader` 用于整体体积，`len("中文甲")=9` 与 `len("中文甲a")=10` 用于纸上 UTF-8 正文字节合同。附件解压/转码后的资源消耗另设边界，前端字符计数不替代服务端。
