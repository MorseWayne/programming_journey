...
当前业务主线为 IM，主参照为 OpenIM。见[项目阅读地图](../../src/docs/platform_engineering/curriculum/im_reference.md)与[固定源码来源清单](./im_reference_manifest.json)。01.01–01.02 保留基础原稿，01.03 起按 IM 主线展开；第二批新增 01.05、02.01、03.01。分工与统稿见[并行批次](./parallel_batches.md)，共用规则见[教学约定](./common_contracts.md)。
...
当前共接入 9 章，不代表 168 个计划单元已全部完成。生成使用已有 DeepTutor BookEngine 和模型配置；HTTP 创建接口的参数兼容问题通过单次 SDK 进程适配绕行，持久服务仍需另行修复。
