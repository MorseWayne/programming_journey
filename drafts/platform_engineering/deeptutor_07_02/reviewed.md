...
不要把 `conversation_counters.next_seq` 移到 `rate:*` 这一类键里。06.12 的 `next_seq` 在同一数据库事务内与成员校验、消息插入共同决定 `seq=9`；Redis `INCR` 即使对**自身**计数正确，也不能自动和数据库 `Commit` 一起回滚，不能未经新协议审阅就替代权威序号分配。[06.12 数据库业务案例](../../../src/docs/platform_engineering/curriculum/06_databases/12_database_business_case.md)
...
按[学习路线](../../../src/docs/platform_engineering/curriculum/learning_path.md)，下一章[07.03 缓存读取与更新](../../../src/docs/platform_engineering/curriculum/07_cache_messaging/03_cache_read_update.md)从 cache-aside 读取与数据库更新的交错讲旧值回填、TTL 和版本；07.04 再处理热点与集中失效。
