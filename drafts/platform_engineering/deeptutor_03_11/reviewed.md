...
能区分 namespace 视图与 cgroup 限额，手算 0.5 核额度和 96% 使用，说明 `memory.current`/Go 堆/RSS 与 `oom_kill` 的不同证据范围，并把网关节流或退出后的连接/消息恢复写成独立业务验收，才算完成第一轮。本章没有执行任何容器或资源实验。下一章 03.12 将用描述符耗尽、磁盘满和信号退出把机制接成完整资源故障案例，然后再进入第十二卷的平台运行课程。
