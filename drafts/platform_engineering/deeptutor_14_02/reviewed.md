...
能手算 `Dq`、三个余弦、两个方向的条件概率和 precision/recall/accuracy，解释为什么训练 12/12 正确不足以证明泛化，并在高相似资料面前仍先审权限与证据，才算完成第一轮。下一章 14.03 将讲 token、embedding、上下文和生成的模型基础，继续把模型输出与 IM 业务事实分开。
