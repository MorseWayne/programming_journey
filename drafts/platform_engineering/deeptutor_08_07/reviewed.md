...
能不看答案画出 r101→r110→r111、解释租约删除为何不能关闭旧 socket、目标为何要接收单调代次及其安装空窗；能把 watch 断线/压缩后的快照重建和 B 的授权补拉写完整，才算学会本章。真实环境还需记录 etcd 配置、协调响应、目标拒旧写、设备效果及故障注入证据。下一章 08.08 将把协调之外的跨服务提交、补偿与状态查询放在同一业务需求下比较。
