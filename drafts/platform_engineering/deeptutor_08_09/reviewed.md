...
能不看答案解释任务与消息身份、两种标记顺序各自的崩溃反例、W1/W2 token 围栏为何不能撤回外部推送，以及三类错误/三次总尝试/设备补拉各自的责任，才算完成第一轮。真实实践要记录提供方幂等协议、任务表并发、失败注入、最老年龄和设备结果，本页没有代替执行。下一章 08.10 将继续讨论成员变化、快照和滚动迁移中的新旧状态解释。
