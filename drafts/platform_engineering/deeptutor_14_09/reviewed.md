...
第一遍能固定六类虚构问题、资料版本/权限和可复跑关键词基线，并解释泄漏与金标签来源；第二遍能按明确分母拆检索、引用、拒答和端到端结果，识别自动裁判偏差与第一个坏边界，才算完成本章。按照本卷路线，接着从[14.04 Transformer 与推理路径](../../../src/docs/platform_engineering/curriculum/14_ai/04_transformer_inference_path.md)深入模型机制，再回本章复核复杂方案。
