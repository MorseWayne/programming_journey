...
能区分 2 字符/6 B 与未知 token 数，解释人工向量和学习 embedding 的差别、玩具下一 token 分布、32 token 纸上窗口与训练/推理差别，并在 `q-01/q-02` 上拒绝把生成文字当合同或权限证据，才算完成第一轮。按本卷先修顺序，接着先读[14.09 评测与数据工程](../../../src/docs/platform_engineering/curriculum/14_ai/09_evaluation_data_engineering.md)的固定问题与关键词基线；Transformer 内部机制在 14.04 深化。
