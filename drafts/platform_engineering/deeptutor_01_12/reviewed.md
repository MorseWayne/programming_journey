...
本章使用完全虚构的 `c-a`、`u-a`、`m-a`。输入为一个不超过 1 MiB 的版本 1 JSON 文件，格式沿用 [01.08](../../../src/docs/platform_engineering/curriculum/01_go/08_standard_library.md)；领域消息和文件适配器沿用 [01.09](../../../src/docs/platform_engineering/curriculum/01_go/09_packages_evolution.md)。程序只读写**本机文件**，不登录、不联网、不发送消息，不把本地导出成功称为服务端持久化、设备接收或用户已读。
...
`list` 保留文件顺序，不偷偷按客户端时间排序。若产品后来要按序号排序，必须引用 [02.05](../../../src/docs/platform_engineering/curriculum/02_algorithms/05_sort_divide.md) 定义比较键、并列规则及对旧输出的兼容影响。`check` 的摘要只说明“本工具接受这份本地文件”，不说明历史完整。
...
使用 [01.09 的文件适配器](../../../src/docs/platform_engineering/curriculum/01_go/09_packages_evolution.md)时，读取过程应保持固定顺序：
