...
能把新 IM 网关的接入需求写成输入、输出、失败反馈和责任清楚的契约，沿 T0–T5 找到第一个断点，并用 `35−12−8=15` 与 `7/10=70%` 说明**条件收益**及其证据缺口，才算完成第一轮。第十二卷至此从制品、容器、集群控制推进到平台服务责任；下一卷 13.01 将从使用方、目标、约束与失败成本重新定义 IM 架构问题。
