...
能不看答案解释三个性质、构造 data-race-free 的双发交错、说清 `-race`/fuzz/门闩/跨节点历史各自能证明什么，并把失败输入变成脱敏回归用例，才算完成第一轮。真实 Go、IM 和故障证据仍由学习者在隔离环境另行取得。下一章 10.08 将检查依赖版本、生成代码、格式与静态分析工具所提供的证据边界。
