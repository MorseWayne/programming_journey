...
能不看答案区分 `db:5432` 与宿主 `127.0.0.1:15432`，说明 Compose 启动顺序、healthcheck 和业务就绪的不同证据范围，逐项反驳四个连接/模式故障候选，并守住当前 S2 与未来 S3/B 设备结果的边界，才算完成第一轮。本章没有运行 Compose 或 IM。下一章 12.04 进入集群的期望状态、控制循环、节点、Pod 与调度。
