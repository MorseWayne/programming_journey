...
5. 用待审 R9 的单条正文 **9 UTF-8 字节**规则把 CI 检查连回 10.01–10.06：`abc` 3 字节、`中文甲` 9 字节、`中文甲a` 10 字节；失败请求不得改动内存列表。先定义该 PR 的拟议预期，再说明单元、接口、构建各自能证明什么以及缺什么。
...
8. 09.11 跨章核对后，在本章开头和检查示例中明确 R9 是将 09.02 当前 6 字节接口**拟改为 9 字节的待审 PR**；纸上 R9 预期不表示当前接口已升级，CI 结果须标出所验证的合同版本。
