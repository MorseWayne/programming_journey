...
4. 原稿一处用字符数 `char_length` 限制正文，容易偏离 S2 的字节上限。审阅稿在 PostgreSQL 教学方言和 UTF-8 编码假定下使用 `octet_length(body) BETWEEN 1 AND 9`；`中文甲` 9 字节、再加 ASCII `a` 为 10 字节。应用层仍须与数据库保持同一业务合同。
